...
public class ServiceTypeController : ControllerBase
{
    ...
    {
        var command = new CreateServiceTypeCommand(request.Title, request.Description, request.Price);
    ...
    public async Task<IActionResult> GetAllServiceTypes()
    {
        ...
        var response = result.Value?.Select(s => new ServiceTypeResponse(s.Id, s.Title, s.Description, s.Price));
        return Ok(response);
    }
    ...
    public async Task<IActionResult> GetServiceTypeById(Guid id)
    {
        ...
        var s = result.Value;
        var response = new ServiceTypeResponse(s.Id, s.Title, s.Description, s.Price);
        return Ok(response);
    }
    ...
    {
        var command = new UpdateServiceTypeCommand(id, request.Title, request.Description, request.Price);
        var result = await _mediator.Send(command);
